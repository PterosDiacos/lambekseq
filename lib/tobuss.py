...
def toBuss(con, pres, cache, tree, parentLinks=None, 
           indent='', space=' ' * 4):
    res = ''
    key = con, *pres
    for links in cache[key]:
        ...
        if (key, links) in tree:
            if len(tree[key, links]) == 2:
                ...
                res += binary_infer(
                    toBuss(sub1[0], sub1[1:], cache, tree, links, indent + space),
                    toBuss(sub2[0], sub2[1:], cache, tree, links, indent + space),
                    con, pres, indent)
            elif len(tree[key, links]) == 1:
                sub, = tree[key, links]
                res += unary_infer(
                    toBuss(sub[0], sub[1:], cache, tree, links, indent + space),
                    con, pres, indent)
        else:
